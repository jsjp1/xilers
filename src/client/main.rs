...
async fn main() {
    let config_content: Config = match std::fs::read_to_string("config.toml") {
        Ok(config_str) => {
            toml::from_str(&config_str).expect("config.toml파일을 파싱하는데 문제가 발생했습니다. 파일 내용을 확인하시기 바랍니다.")
        },
        Err(e) => {
            println!("파일이 존재하지 않습니다. 기본 설정을 적용합니다: {}", e.to_string());
            Config {
                server: ServerConfig {
    ...
    let master_addr = format!("{}:{}", config_content.server.master_ip, config_content.server.master_port);
    ...
    if env::args().nth(1).unwrap() == "cli" {
        ...
        let mut cli: Cli = Interface::new(master_addr, config_content.client.listen_port, config_content.client.file_storage.to_string());
        cli.entry().await;
